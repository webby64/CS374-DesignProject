...
function ViewMainModal(buttonPopup, setButtonPopup) {
  ...
  console.log("the list of # books",numBooksReadList)
  var pagesState = {
    labels: chartLabelsState ,
    datasets: [
      {
  ...
  return(
    <Popup trigger={buttonPopup} setTrigger = {setButtonPopup}>
      <Button id = "bookList-btn" className = "btn-light"variant="light" onClick={() => setState("bookList")}><FaListAlt/> Book List</Button>
      ...
      {whichModalState === 'mainModal' && (
        <div id="avatarAndProgressRing">
          ...
          <Button ref={clickChangeFavAuthor} 
              ...
              onClick = {() => {if (favAuthorChanged !== "") {setFavAuthorState(favAuthorChanged); writefavAuthortoDB(favAuthorChanged)} else alert("Please Insert the Author Name") }}></Button>
      ...
      {whichModalState === 'wishList' && (
         ...
         <div className = "theLists">
          ...
          <Button variant = "secondary" className='btn-sm' id = "addWish-btn" 
                  onClick = {() => {if (addedItem !== "") {setWishList(wishListState.concat(addedItem)); 
                                      writewishListtoDB(wishListState.concat(addedItem));setAddedItem('')} 
                                    else alert("Please add the book title First")}}
                  ref = {clickAdd}
                  >+</Button>
          </div>
         </div>
      ...
      {whichModalState === 'stats' && (
         <div>      
          ...
          <Button id="booksRead-btn" style = {{ border: "3px solid green"}} variant="secondary" size="sm"onClick={() => setState("stats")}>Books Read</Button>
          {/* <Button id="uniqueWords" variant="light" size="sm"onClick={() => setState("uniqueWordsStat")}>Unique Words</Button> */}
          <Button id="pagesRead-btn" variant="secondary"  size="sm"onClick={() => setState("pagesReadStat")}>Pages Read</Button>
       </div>
      )}
      ...
      {whichModalState === 'pagesReadStat' && (
         <div> 
          ...
          <Button id="booksRead-btn" variant="secondary"  size="sm"onClick={() => setState("stats")}>Books Read</Button>
          {/* <Button id="uniqueWords" variant="light" size="sm"onClick={() => setState("uniqueWordsStat")}>Unique Words</Button> */}
          <Button id="pagesRead-btn" style = {{ border: "3px solid green"}} variant="secondary"  size="sm"onClick={() => setState("pagesReadStat")}>Pages Read</Button>
       </div>
      )}
    </Popup>
  )
...
function ProfileImage() {
  ...
  const currentUserID = "2tnFZtmvdyZMo3jNzoeizfDPU1v1" // useAuth().currentUser.uid;
